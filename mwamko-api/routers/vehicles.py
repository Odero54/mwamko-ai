...
router = APIRouter(prefix="/vehicles", tags=["Resource Management - Vehicles"])
...
async def create_vehicle(
    ...
    existing_vehicle = (
        db.query(RescueVehicles)
        .filter(RescueVehicles.registration_plate == vehicle_data.registration_plate)
        .first()
    )
    ...
    existing_type = (
        db.query(RescueVehicles)
        .filter(RescueVehicles.vehicle_type == vehicle_data.vehicle_type)
        .first()
    )
...
async def get_vehicles(
    ...
    vehicles = (
        query.order_by(RescueVehicles.last_update_ts.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def get_vehicle(
    ...
    """
    vehicle = (
        db.query(RescueVehicles).filter(RescueVehicles.vehicle_id == vehicle_id).first()
    )
...
async def get_available_vehicles(
    ...
    Available to: All authenticated users
    """
    query = db.query(RescueVehicles).filter(
        RescueVehicles.current_status == "AVAILABLE"
    )
...
async def get_vehicle_status_summary(
    ...
    status_counts = (
        ...
        .group_by(RescueVehicles.current_status)
        .all()
    )
