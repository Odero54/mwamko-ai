...
from models import ArchivedRoutes, EmergencyCases, Routes, Users
from sqlalchemy.orm import Session

router = APIRouter(prefix="/cases", tags=["Route Optimization"])
...
async def get_latest_route(
    ...
    return RouteSummary(
        ...
        total_segments=(
            len(latest_route.route_segments) if latest_route.route_segments else 0
        ),
    )
...
async def archive_route(
    ...
    unresolved_cases = (
        ...
        .filter(
            EmergencyCases.case_id.in_(case_ids), EmergencyCases.status != "RESOLVED"
        )
        .all()
    )
...
async def get_archived_routes(
    ...
    archived_routes = (
        db.query(ArchivedRoutes)
        .order_by(ArchivedRoutes.archived_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def get_case_ids_from_route(
    ...
    for node_id in optimized_sequence[1:]:  # Skip the first node (start point)
        ...
        case = (
            db.query(EmergencyCases).filter(EmergencyCases.location.isnot(None)).first()
        )  # This is simplified - you might need a more complex spatial query
...
async def get_resolved_by_user(db: Session, case_ids: List[int]) -> str:
    ...
    latest_case = (
        db.query(EmergencyCases)
        .filter(EmergencyCases.case_id.in_(case_ids))
