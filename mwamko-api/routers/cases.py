...
router = APIRouter(prefix="/api/v1", tags=["Emergency Case Management"])
...
async def get_emergency_cases(
    ...
    status_filter: Optional[str] = Query(
        None, description="Filter by status (PENDING, IN PROGRESS, RESOLVED)"
    ),
    severity_filter: Optional[str] = Query(None, description="Filter by severity"),
    disaster_type_filter: Optional[str] = Query(
        None, description="Filter by disaster type"
    ),
    ...
    cases = (
        query.order_by(EmergencyCases.created_at.desc()).offset(skip).limit(limit).all()
    )
...
async def get_emergency_case(
    ...
    if not case:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Emergency case not found"
        )
...
async def update_emergency_case(
    ...
    if (
        current_user["role"] == "Rescue Team"
        and case_update.status == "RESOLVED"
        and case.assigned_team_id != current_user["id"]
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
