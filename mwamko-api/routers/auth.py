...
from models import Invite, Users
...
def create_access_token(
    email: str, user_id: int, county: str, role: str, expires_delta: timedelta
):
...
async def create_user(db: db_dependency, create_user_request: CreateUserRequest):
    ...
    if (
        ...
        .filter(
            ...
            | (Users.id_number == create_user_request.id_number)
        )
        .first()
    ):
        raise HTTPException(
...
@router.post(
    "/admin/invite", response_model=InviteResponse, status_code=status.HTTP_201_CREATED
)
async def invite_user(
    ...
    existing_invite = (
        db.query(Invite)
        .filter(Invite.email == invite_data.email, Invite.status == "PENDING")
        .first()
    )
    ...
    background_tasks.add_task(
        send_invite_email, invite_data.email, invite_token, coordinator_name
    )
...
async def accept_invite(accept_data: AcceptInviteRequest, db: db_dependency):
    """
    ...
    invite = (
        db.query(Invite)
        .filter(Invite.token == accept_data.token, Invite.status == "PENDING")
        .first()
    )
    ...
    if (
        db.query(Users)
        .filter((Users.email == invite.email) | (Users.id_number == invite.id_number))
        .first()
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
async def get_pending_users(
    ...
    pending_users = (
        db.query(Users)
        .filter(Users.county == coordinator_county, Users.role == "PENDING")
        .all()
    )
...
async def get_invites(
    ...
    invites = (
        db.query(Invite)
        .filter(Invite.invited_by == coordinator["id"])
