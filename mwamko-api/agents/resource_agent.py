...
class ResourceAllocationAgent:
    ...
    async def optimize_resource_allocation(
        self, emergencies: List[EmergencyCases]
    ) -> Dict[str, Any]:
        ...
        available_teams = (
            self.db.query(Users)
            .filter(Users.role == "Rescue Team", Users.is_active == True)
            .all()
        )
    ...
    def _calculate_priority_score(self, emergency: EmergencyCases) -> float:
        ...
        time_factor = (
            min((datetime.now() - emergency.created_at).total_seconds() / 3600, 24) / 24
        )

        return base_score * disaster_multiplier * (1 + time_factor)

    def _find_best_team(
        self, emergency: EmergencyCases, available_teams: List[Users]
    ) -> Users:
